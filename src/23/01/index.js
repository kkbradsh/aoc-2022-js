...
export const process = async (file) => {
  ...
  move(map);
  // Return: find the rectangle that contains the elves and count how many spaces do not contain elves
  let minRow, maxRow, minCol, maxCol;
  for (let key of map.keys()) {
    const [row, col] = key.split(',');
    minRow = Math.min(minRow !== undefined ? minRow : row, row);
    maxRow = Math.max(maxRow !== undefined ? maxRow : row, row);
    minCol = Math.min(minCol !== undefined ? minCol : col, col);
    maxCol = Math.max(maxCol !== undefined ? maxCol : col, col);
  }
  const positions = (maxRow - minRow + 1) * (maxCol - minCol + 1);
  return positions - map.size;
};
